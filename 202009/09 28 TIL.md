 ...
 * 자바스크립트는 이러한 반복(iteration) 작업을 위한 여러 수단들(iteratees)을 제공합니다.
 *  반복문(for, for of, for in), map(), filter()
...
_.take = function (arr, n) {
  // TODO: 여기에 코드를 작성합니다.
  let result = [];
    if(n === undefined || n < result.length) {
      return result;
    } else {
...
_.reduce = function (arr, iteratee, initVal) {
  ...
  if(initVal === undefined ){
    ...
    _.each(arr,function(item,index){
      if(index > 0){
        initVal = iteratee(initVal,arr[index],index,arr);
      }
    });
    return initVal;
    } else {
